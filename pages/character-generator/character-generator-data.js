...
// Each setting registers itself by pushing onto this array from its own
// file in settings/ (settings/generic.js, settings/forgotten-realms.js,
// ...) -- adding a setting is adding a file and a <script> tag for it in
// character-generator.html, not editing this one. Nothing here depends on
// registration order: DEFAULT_SETTING_ID, not SETTINGS[0], is what marks
// generic as the fallback (see settingFor below and its three call sites
// in character-generator.js).
const SETTINGS = [];
const DEFAULT_SETTING_ID = "generic";

function settingFor(id) {
  return (
    SETTINGS.find((entry) => entry.id === id) ||
    SETTINGS.find((entry) => entry.id === DEFAULT_SETTING_ID)
  );
}
